...
- **Build** (optional): `deno task build` — then **Start**:
  `deno task start`
- **Test**: `deno task test` or `./scripts/dev.sh test` (DB
  env). Before push: `deno task pre-push` (runs via dev.sh).
...
See [CONTRIBUTING.md](CONTRIBUTING.md). For general or
...
[Contributor Covenant](CODE_OF_CONDUCT.md). For CoC
concerns:
