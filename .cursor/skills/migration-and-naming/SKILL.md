...
/rules-summary migration) and apply the listed § for this
session. Keep the output in context.

## §J — Migration boundary

- Plan before execute: write a migration plan (current file
  → target filename and content responsibility); do not
  rename or split without this mapping.
- Execute order: create all new rule/files first, then
  delete old ones; one logical migration per commit.
- New names follow §D and §E. Adding/refactoring
  .cursor/rules does not require BACKLOG.md change.
...
- [ ] Migration plan written (current → target) before any
      renames.
- [ ] New files created first, then old files removed.
- [ ] New names follow §D, §E; directory structure follows
      §F.
