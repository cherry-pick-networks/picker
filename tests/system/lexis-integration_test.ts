...
import { LexisEntrySchema } from "#system/lexis/lexis.schema.ts";
import { hasDbEnv } from "./db-env_test.ts";

const handler = (req: Request) => app.fetch(req);
const dbOpts = () => ({ sanitizeResources: false, ignore: !hasDbEnv() });
...
Deno.test(
  ...
  async () => {
    const res = await handler(
      new Request(url("/lexis/entries", { q: "워드마스터 중등 실력 1일차" })),
    );
    assertEquals(res.status, 200);
    const body = (await res.json()) as { entries: unknown[] };
    assertEquals(Array.isArray(body.entries), true);
...
Deno.test(
  ...
  async () => {
    ...
    try {
      const res = await handler(
        new Request(url("/lexis/entries", { q: "그 책 1일차만" })),
      );
      assertEquals(res.status, 200);
      const body = (await res.json()) as { entries: unknown[] };
      assertEquals(Array.isArray(body.entries), true);
