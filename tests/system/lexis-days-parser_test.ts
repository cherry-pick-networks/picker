/**
 * days-parser: regex extraction of day numbers from Korean utterance.
 */

import { assertEquals } from "@std/assert";
import { parseDays, parseDaysWithRegex } from "#system/lexis/days-parser.ts";

Deno.test("parseDaysWithRegex single day", () => {
  assertEquals(parseDaysWithRegex("17일차"), [17]);
  assertEquals(parseDaysWithRegex("워드마스터 17일차 단어"), [17]);
});

Deno.test("parseDaysWithRegex range with middle dot", () => {
  assertEquals(parseDaysWithRegex("17·18일차"), [17, 18]);
  assertEquals(parseDaysWithRegex("17·18일차 단어 알려줘"), [17, 18]);
});

Deno.test("parseDaysWithRegex range hyphen and tilde", () => {
  assertEquals(parseDaysWithRegex("1-5일차"), [1, 2, 3, 4, 5]);
