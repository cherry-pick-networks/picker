...
} from '#system/content/material/matcherConfig.ts';
...
function setLexisEnv(): void {
  Deno.env.set(
    'SOURCE_META_MIDDLE_INTERMEDIATE',
    JSON.stringify({
      type: 'book',
      metadata: {
        title: '워드마스터 중등 실력',
  ...
  Deno.env.set(
    'SOURCE_META_MIDDLE_BASIC',
    JSON.stringify({
      type: 'book',
      metadata: {
        title: '워드마스터 중등 Basic',
        keywords: ['중등 Basic'],
      },
    }),
  );
  Deno.env.set(
    'SOURCE_META_HIGH_BASIC',
    JSON.stringify({
      type: 'book',
      metadata: {
        title: '워드마스터 고등 Basic',
        keywords: ['고등 Basic'],
      },
    }),
  );
