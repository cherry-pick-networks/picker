//
// POST /content/recommend/rag: 400 when query missing; 200 with mock.
...
Deno.test(
  'POST /content/recommend/rag returns 400 when query empty',
  async () => {
    ...
    try {
      const res = await handler(
        new Request(
          'http://localhost/content/recommend/rag',
          {
            method: 'POST',
...
Deno.test(
  'POST /content/recommend/rag returns 200 and recommendations (mock, no expand)',
  dbOpts(),
  async () => {
    Deno.env.set('SEMANTIC_EMBED_MOCK', '1');
    Deno.env.set('TEST_SKIP_ENTRA_AUTH', '1');
