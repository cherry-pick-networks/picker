...
function loadPathConfig(): { root: string; paths: Record<string, string> } {
  const cwd = Deno.cwd();
  let path = findPathConfig(cwd);
  if (!path) {
    const scriptDir = dirname(fromFileUrl(import.meta.url));
    const fallbackPath = join(scriptDir, 'path-config.json');
    try {
      Deno.statSync(fallbackPath);
      path = fallbackPath;
    } catch {
      throw new Error(
  ...
  const root =
    (pathDir.endsWith('/config') || pathDir.endsWith('\\config')) &&
    pathDir.includes('pipeline')
      ? dirname(dirname(pathDir))
      : dirname(pathDir);
