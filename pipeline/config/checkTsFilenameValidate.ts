...
function isTestDirPath(rel: string): boolean {
  return rel.startsWith('api/test/') || rel.startsWith('pipeline/test/');
}
...
function validateRestTestsOrSystem(
  ...
  if (isTestDirPath(rel)) return validateTests(base);
  if (rel.startsWith('api/') || rel.startsWith('application/')) {
    return validateSystem(rel, base);
  }
  return null;
}

//  pipeline/config/: base filename camelCase or PascalCase per §E.
function validateRestShared(
  rel: string,
  base: string,
): string | null {
  if (!rel.startsWith('pipeline/config/')) {
    return null;
  }
  return checkTsBaseName(base);
}
