...
// excluded). Run: deno task dependency-check
//
// Allowlist must match sharepoint/context/documentation/MANUAL.md
// § Domain dependency.
//
...
} from './checkDomainDepsHelpers.ts';
  ...
  for (const domain of DOMAINS) {
    ...
    for await (const filePath of findTsFiles(domainDir)) {
      ...
      for (const imp of extractImports(content)) {
        const toDomain = parseCrossDomainEdge(
          imp,
          domain,
          DOMAINS,
        );
  ...
  if (disallowed.length > 0) {
    console.error(
      'Domain dependency violation: cross-domain edge not in allowed ' +
        "matrix. Update MANUAL.md and this script's ALLOWED_EDGES.",
    );
