...
import plugin from '#pipeline/config/functionLengthLintPlugin.ts';
...
Deno.test('function-length: single return allowed (await)', () => {
  const code = `
async function get() {
...
Deno.test('function-length: single return allowed (chain)', () => {
  const code = `
function get() {
  return fetch("/api")
    .then((r) => r.json())
...
Deno.test('function-length: single return allowed (plain)', () => {
