...
//  Retrievability (0–1) from elapsed days and stability.
export function retrievabilityAt(
  elapsedDays: number,
  stability: number,
): number {
  return forgettingCurve(elapsedDays, stability);
}
...
function nextDifficulty(
  ...
  return clamp(
    meanReversion(initDifficulty(4), next),
    1,
    10,
  );
}

function _shortTermStability(
  stability: number,
  rating: number,
): number {
  const g = clamp(rating, 1, 4);
...
// function-length-ignore — interval + next date.
...
export function scheduleNew(
  ...
  return {
    nextDue: next,
    state: { difficulty: d, stability: s },
  };
}

// function-length-ignore — recall/forget branches.
...
export function scheduleReview(
  ...
  const elapsedDays = Math.max(
    0,
    (now.getTime() - lastReviewedAt.getTime()) /
      (24 * 60 * 60 * 1000),
  );
  ...
  return {
    nextDue: next,
    state: { difficulty: dNext, stability: sNext },
  };
}
