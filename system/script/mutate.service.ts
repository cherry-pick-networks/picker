 ...
 * replace → writeScript once. Uses scripts.store, governance, mutate.schema,
 * llm.client. MAB·DAG remain local (boundary § Mutation boundary).
 */
...
import type {
  MutateRequest,
  MutateResponse,
} from "#system/script/mutate.schema.ts";
import { readScript, writeScript } from "#system/script/scripts.store.ts";
