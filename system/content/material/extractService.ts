...
} from '#system/governance/allowlistTypes.ts';
import { getAllowlistDataOrLoad } from '#system/governance/allowlistData.ts';
import type { Source } from './sourceSchema.ts';
import { ContentLlmService } from '#system/content/ContentLlmService.ts';
import { getSource } from './service.ts';
...
async function validateExtractConceptIds(
  ids: string[],
): Promise<void> {
  ...
  if (invalid.length > 0) {
    throw new Error(
      `Invalid concept IDs: ${invalid.join(', ')}`,
    );
  }
}

async function validateExtractSubjectId(
  id: string | undefined,
): Promise<void> {
...
function persistExtract(
  ...
  return ContentStores.sourceStore
    .setSource(
...
async function validateExtractIds(
  concept_ids: string[],
  subject_id: string | undefined,
): Promise<void> {
...
async function validateAndPersist(
  ...
  await validateExtractIds(concept_ids, subject_id);
  return persistExtract(source, concept_ids, subject_id);
}

function runExtractFromLlm(source: Source) {
  return ContentLlmService.extractConcepts(
    source.body as string,
  );
}

async function runExtractAndSave(
  source: Source,
): Promise<ExtractOk> {
  const llm = await runExtractFromLlm(source);
