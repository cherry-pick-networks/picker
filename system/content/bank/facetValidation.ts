...
function buildFacetChecks(
  ...
  const raw = [
    body.subject_ids?.length
      ? (['subject', body.subject_ids] as const)
      : null,
    ...
    body.cognitive_level_id
      ...
      : null,
    body.context_ids?.length
      ? (['context', body.context_ids] as const)
      : null,
    body.concept_id
      ? (['concept', [body.concept_id]] as const)
      : null,
  ].filter((x): x is NonNullable<typeof x> => x != null);
  return raw as FacetCheck[];
}

function checkTotalLimit(checks: FacetCheck[]): void {
...
async function validateChecksAgainstAllowlist(
  checks: FacetCheck[],
): Promise<void> {
...
export async function validateItemFacets(
  ...
  checkTotalLimit(checks);
  await validateChecksAgainstAllowlist(checks);
}
