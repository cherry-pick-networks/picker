...
// function-length-ignore — single conditional (store.md §P)
...
function collectRange(seen: Set<number>, s: string): void {
  for (const m of s.matchAll(RANGE_DAY)) {
...
function collectSingles(seen: Set<number>, s: string): void {
  for (const m of s.matchAll(SINGLE_DAY)) {
    addDay(seen, parseInt(m[1], 10));
  }
}

export function parseDaysWithRegex(utterance: string): number[] | null {
...
export function parseDays(utterance: string): DaysParseResult | { ok: false } {
  const days = parseDaysWithRegex(utterance);
  if (days == null || days.length === 0) return { ok: false };
  return { ok: true, days };
}
