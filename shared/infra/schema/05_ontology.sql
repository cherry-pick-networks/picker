-- Ontology: SKOS-style concept schemes and concepts (facets, DDC).
-- Prerequisite: run after 05_knowledge.sql if both present. Seed: shared/infra/seed/.
...
CREATE TABLE IF NOT EXISTS concept_relation (
  ...
  CONSTRAINT concept_relation_type_check CHECK (
    relation_type IN ('broader', 'narrower', 'related', 'exactMatch')
  )
);
