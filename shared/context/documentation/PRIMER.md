...
- **Where**: `shared/context/HANDOFF.md` (linked from
  README).
...
RULESET.md §P and shared/context/documentation/MANUAL.md (§P
pattern guide).
...
POST /content/worksheets. Entry: system/routes/content.ts."
...
- **Output**: Line "Applicable sections: §X, §Y, …" then
  ...
  text stays in `shared/context/RULESET.md`; Rule index is
...
| **codebase-explorer**   | explore               | Codebase exploration, file/pattern/keyword search, structure discovery. Respect pathConfig and RULESET § structure. See `shared/context/documentation/codebase-explorer-spec.md`. |
| **shell-runner**        | shell                 | Terminal commands (deno task, git, dev.sh). pre-push, rules:summary, clean-branches. Does not run destructive commands (RULESET.md §7). See `shared/context/documentation/shell-runner-spec.md`. |
...
`shared/context/documentation/compliance-verifier-work-instructions.md` if defined.
...
- **Rule context**: For implementation, refactor, or
  docs-editing sessions, attach `@shared/context/RULESET.md`
  or `@shared/context/SUMMARY.md` in the first message, or
...
- **Script**: `shared/context/scripts/setup.sh`
...
- To bootstrap RULESET.md and overview in a new
  ...
  `shared/context/RULESET.md` (e.g. §1–§5 and Rule index
  skeleton) and `shared/context/CONTEXT.md`. Ensure
  `deno task rules:summary -- all` works (add the task in
  deno.json if needed). Optionally add a task `init-prompt`
  that runs a script in `shared/context/scripts/` to copy
  templates.
...
one, add it to this guide or to `shared/context/RULESET.md`
as appropriate.
...
practice to this guide or to `shared/context/RULESET.md`
