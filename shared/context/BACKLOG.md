...
shared/context/documentation/openapi.yaml; route list:
system/routes.ts. Todo-check validates routes against
openapi.yaml. Domain consolidation: see
shared/context/documentation/MANUAL.md § Domains and API
paths (6 + App).

**Final implementation goal**: See
`shared/context/CONTEXT.md` §5 for the one-line goal, target
...
| **system/routes.ts**      | Route list (ROUTES) and registerRoutes(app); todo-check validates against openapi.yaml.                    |
| **system/app/**           | Route registration, auth. Imports domain endpoints only.                                                   |
| **system/identity/**      | Actors (actor_id, display_name, level, progress) + actor schedule (due, plan, items, review).              |
| **system/governance/**    | Scripts store, mutate, allowlist/ontology (data only). Replaces script + concept.                          |
| **system/content/**       | Source CRUD, extract; material (source + lexis entries); content item CRUD; view aggregation (source-dashboard). |
| **system/governance/**    | Scripts store, mutate, allowlist/ontology; audit log (e.g. e2e_runs).                                      |
| **shared/runtime/store/** | Target path for self-edit; read and write only via Governance-verified flow.                               |
| **system/infra/**         | Postgres client (`getPg()`), SQL loader, schema applier, seed runners, KV storage, batch (e.g. dbListAll). |
...
shared/context/documentation/MANUAL.md § API and Copilot
scope. Keep openapi.yaml and system/routes.ts aligned when
...
- **When mutating**: Use only path `shared/runtime/store/`.
...
- **PostgreSQL** — single storage backend. Client:
  `system/infra/pgClient.ts` (`getPg()`). Domain stores:
  identity, content; KV and batch in infra. DDL data under
  `shared/infra/schema/` (see MANUAL.md). Apply:
  `deno task db:schema` (system/infra/applySchema.ts).
- **File-based data** — identity_index.toml under
  `shared/record/reference/` may remain as legacy; Identity
  domain can migrate to DB (actor table).
- **Worksheet prompt templates** — read-only from
  `shared/runtime/store/` (e.g. docs/contract/);
  Governance-verified read.
- **Change audit log** — stored under `system/governance/`
...
- **Ontology (Todo 1)** — DDL:
  `shared/infra/schema/06_ontology.sql`. Seed data:
  `shared/infra/seed/ontology/`. Seed task:
