...
//
// Todo check helpers: parse openapi paths or todo doc, map file
// paths to routes, extract handler methods, walk router dir.
// Used by checkTodo.ts.
//
import type { Route } from './checkTodoLibOpenApi.ts';

export type { Route };
export {
  openApiPathToHono,
  parseOpenApiPaths,
} from './checkTodoLibOpenApi.ts';

export function parseScopeApiTable(
  content: string,
): Route[] {
  const routes: Route[] = [];
...
export function extractMethodsFromHandler(
  content: string,
): string[] {
  const methods: string[] = [];
  const re =
    /handler\.\s*(GET|POST|PUT|PATCH|DELETE)\s*[\(\{]/g;
...
export async function walkRouterFiles(
  ...
  for await (const e of Deno.readDir(dir)) {
    ...
    if (e.isDirectory) {
      if (
        e.name.startsWith('_') || e.name === 'node_modules'
      ) continue;
