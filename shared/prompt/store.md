...
not rely on manual 80-char counting. Line length: keep lines to 80 characters
or fewer (strict); exceptions only where documented (e.g. long URLs in
comments). One effective line = 80 character units
...
(try/catch, switch, block-bodied if); fix or extract before committing. Then
run `deno fmt` and `deno task line-length-check`; if line-length fails (e.g. on
...
`Deno.test("...", ...)` or any string literal exceeds 80 chars, split the
string with `+` concatenation across lines or use a template literal across
lines; keep each line under 80 chars.
