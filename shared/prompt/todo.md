...
| GET    | `/schedule/plan/weekly`            | Weekly lesson plan. Query: actor_id, week_start, optional level, slot_dates. Responds { week_start, week_end, new_units, review_units, slots?: { date, new_unit, review_units }[] }. 3 new units/week (1 per session). |
| GET    | `/schedule/plan/annual`            | Annual curriculum. Query: actor_id, year (or start), optional level. Responds { weeks: { week_start, slots: { slot_index, new_unit }[] }[] }. Optional.                                                                              |
