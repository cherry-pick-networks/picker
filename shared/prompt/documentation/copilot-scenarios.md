...
3. **Schedule** — `GET /schedule/due?actor_id=&from=&to=` for due items;
   `GET /schedule/plan/weekly` for planning; `POST /schedule/items` to create.
4. **Content** — Create items with `POST /content/items` (one per item); create
   worksheet with `POST /content/worksheets` (body: `title`, `item_ids[]`).
...
1. Use `POST /content/worksheets/build-prompt` or a local prompt to get
   topic/context for the LLM.
