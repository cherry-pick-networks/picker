...
**Scope**: TypeScript source (`**/*.ts`); exclude node_modules, vendor,
generated output. File-length rule does not apply to test files (paths ending
with `_test.ts` or under `tests/`); line-length (100 chars) still applies.
...
- **File length**: Each file must have ≤ 100 effective lines. Effective lines =
  sum over all physical lines of ceil(line length / 100); empty line = 0.
...
- More than one use case or operation in the feature → one file per use
  case/operation (e.g. create vs list).
...
- Public surface would have more than 3–5 exports (or 1 default + 3+ named) →
  plan sibling modules so each file has one clear responsibility and a small set
  of exports.
...
- Aim under 100 effective lines when adding a file; if the planned content would
  clearly exceed that, split before writing (e.g. one file per sub-operation or
  per group of related helpers).
...
- Would the module mix layers (e.g. endpoint + service)? → Yes → one file per
  layer.
- Can this file be described in one short phrase? → No → split by responsibility
  before writing.
...
- Add an index.ts (or existing barrel) only when callers import more than one of
  the split modules; do not add a barrel that only re-exports a single file.
