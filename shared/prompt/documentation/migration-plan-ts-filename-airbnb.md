...
Per store.md §J. Goal: filename = default export name; camelCase for functions/modules; no dot in filename.

## Rule change

- **Before**: `[name].[suffix].ts` (e.g. `profileEndpoint.ts`) or kebab-only (e.g. `contentWorksheetInit.ts`).
- **After**: Single token camelCase (e.g. `profileEndpoint.ts`, `contentWorksheetInit.ts`). No dot; no hyphen in filename.
