...
| **system/store/**        | Storage access (e.g. Deno KV via `getKv()`). Profile/progress KV under prefixes `profile`, `progress`. Content items/worksheets under `content` (item, worksheet). Source records under `source`. File-based UUID v7 storage under `data/` (extracted, identity) and index JSONs via system/store/data.ts. |
...
- **File-based data** — UUID v7–named JSON files under `data/extracted/` (from original documents) and `data/identity/` (PII). Index files: `data/extracted-data-index.json`, `data/identity-index.json`. Populated by migration from `.old`; read/write via system/store/data.ts.
