...
- **global-directory-boundary / global-document-format / global-document-naming**: Single AI context lives under **shared/prompt/** (prefix = shared [Scope], infix = prompt [Entity]); one file, e.g. **context.md**. No extra tier; max 3 levels respected.
...
- **global-document-language**: All content inside **shared/prompt/context.md** (and any other project docs) must be in **English** (code, comments, UI strings, docs).
...
- **Single context file**: **shared/prompt/context.md**  
  - All rules and context for AI/tools live here. Content in **English** (global-document-language).
- **This plan**: **shared/prompt/implementation-plan.md** (recommended)  
...
| Purpose | Approach |
|---------|----------|
| Project rules and context | Single file **shared/prompt/context.md**; each tool references or attaches it. |
...
| Tips and workflow | Use external tips as reference only; write only the chosen habits into **shared/prompt/context.md**. |
...
- **0.1** Confirm the single-context path: **shared/prompt/context.md** (no `docs/`).
...
- **1.2** Create **shared/prompt/context.md** and fill it with (in English):
...
**Deliverable**: **shared/prompt/context.md** as the single source of truth for rules and context.

---

### Phase 2: Tool-side “reference only” setup

- **2.1** **Cursor**: Configure Rules to read **shared/prompt/context.md** (or a short citation). Do not copy long rule text into Rules.
- **2.2** **Other AI tools**: Document that new conversations should attach **shared/prompt/context.md** or pass its path (e.g. in README or CONTRIBUTING).
...
- **3.1** Problem decomposition: break large work into small steps; add a one-line note in **shared/prompt/context.md**.
...
**Deliverable**: Git/PR/CI and safety policy live only in **shared/prompt/context.md**.
...
**Deliverable**: MCP/Playwright/path/backoff guidance in **shared/prompt/context.md** only.
...
- **6.2** Handoff: use a single markdown file (e.g. **shared/prompt/handoff.md** or **HANDOFF.md** at repo root) with goal, progress, what was tried/failed, next steps. New sessions attach only that file. Document this in context.md.
- **6.3** Branching experiments: document “branch/fork conversation or record branch point in handoff” in context.md.

**Deliverable**: Handoff template and short handoff/branch rules in **shared/prompt/context.md**. Handoff file path and name decided (stay within directory rules if under **shared/**).
...
**Deliverable**: I/O habits described only in **shared/prompt/context.md**.
...
**Deliverable**: Quality and safety rules only in **shared/prompt/context.md**.

---

### Phase 9: Maintenance

- **9.1** Review **shared/prompt/context.md** periodically; add “repeated” instructions when they appear; document the review cadence in the file or in this plan.
- **9.2** Keep a single source of truth: all new rules and habits go into **shared/prompt/context.md**; do not duplicate in tool-specific configs.
- **9.3** Use external tips only as reference; write only the chosen practices into context.md.

**Deliverable**: Maintenance procedure documented (e.g. in **shared/prompt/implementation-plan.md** or at the end of context.md).
...
| # | Item | Notes |
|---|------|-------|
| 1 | Path **shared/prompt/context.md** fixed | Single source; directory rule compliant |
| 2 | **shared/prompt/context.md** created with project description, stack, structure, run/test, rules (English) | global-document-language |
| 3 | Cursor Rules reference **shared/prompt/context.md** only (no long copy-paste) | Reference only |
| 4 | Other tools: “attach shared/prompt/context.md or pass path” documented | One place to update |
| 5 | Git/gh/PR/CI and safety policy in **shared/prompt/context.md** | CLI-only, model-agnostic |
| 6 | Handoff template and path (e.g. shared/prompt/handoff.md or root HANDOFF.md) | Document in context.md |
| 7 | MCP/Playwright usage described in **shared/prompt/context.md** | Protocol/CLI only |
| 8 | Review cadence for **shared/prompt/context.md** defined | e.g. quarterly |
...
- **Rules and context**: One file, **shared/prompt/context.md** (English); all tools reference or attach it.
