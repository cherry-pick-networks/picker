/**
 * Verify concept_relation forms a DAG (no cycles). Exit 0 if acyclic, 1 if
 * cycle found. Requires DATABASE_URL or PG* env; run after seed:ontology.
 * Task: deno task ontology-acyclic-check
 */
...
async function main(): Promise<void> {
  ...
  const { rows } = await sql.queryObject<Edge>(SQL_ALL_EDGES);
  const cycle = findCycle(rows);
  if (cycle != null) {
    console.error("Ontology acyclic check failed: cycle in concept_relation:");
    console.error(cycle.join(" -> "));
    Deno.exit(1);
  }
  console.log("Ontology acyclic check passed (concept_relation is a DAG).");
}
