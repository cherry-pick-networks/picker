...
then implement. Domain consolidation: see
...
Paths align with six domains + App. Identity = actors (display_name, level,
progress). Mirror = backup/sync only.
...
- **PostgreSQL** — single storage backend. Client: `shared/infra/pg.client.ts`
  ...
  reference.md).
- **File-based data** — identity-index.toml under `shared/record/reference/` may
  remain as legacy; Identity domain can migrate to DB (actor table).
