...
- **Log artifact storage**: `system/log/` holds log JSON (boundary.md). `system/log/e2e-runs.json` stores E2E run history (schemaVersion, runs[]). `system/store/log.ts` exports `getLogDir()`, `getE2eRunsPath()`, `readE2eRuns()`, `appendE2eRun()`. `deno task test:e2e-record` runs E2E tests and appends one run entry (max 20 runs kept).
