...
import { listGrammarUnits } from './grammarService.ts';
...
async function buildFromCurriculumStore(
  ...
  const weeks: AnnualWeek[] = Array.from(
    { length: 52 },
    (_, i) => ({
      week_number: i + 1,
...
// function-length-ignore — 52 weeks × slots loop.
  ...
  for (let w = 1; w <= 52; w++) {
    ...
    for (let si = 0; si < SLOTS_PER_WEEK; si++) {
      ...
      slots.push({
        slot_index: si,
        new_unit: u
          ? { source_id: u.source_id, unit_id: u.unit_id }
          : null,
      });
    }
...
export async function getAnnualCurriculum(options: {
  ...
  if (options.level) {
    return await buildFromCurriculumStore(options.level);
  }
