...
import type { ScheduleItemPayload } from "#identity/config/jobs/schema.ts";

const sqlDir = new URL("./", import.meta.url);
const SQL_GET = await loadSql(
  sqlDir,
  'get_schedule_item.sql',
);
const SQL_SET = await loadSql(
  sqlDir,
  'set_schedule_item.sql',
);
...
// function-length-ignore — get + parse payload.
...
export async function setScheduleItem(
  row: ScheduleItemRow,
): Promise<void> {
  const pg = await getPg();
