...
### Test file names (C1/test/)

Under `api/test/` and `pipeline/test/`, every test `.ts` file
must be `[name]_test.ts` (Deno convention). The **name** part
uses camelCase (e.g. `scriptsStore_test.ts`,
`sourceExtractEndpoint_test.ts`). Non-test helpers (e.g. under
api/test/) are allowed without `_test` suffix. Validated by
`deno task ts-filename-check`.

### TypeScript filename — To Do / Not To Do (WI-CODE-003)

Scope: `api/**/*.ts`, `api/test/**/*.ts`,
`pipeline/test/**/*.ts`, `pipeline/config/*.ts`. Root and
system root `.ts`:
...
- **Tests**: `[name]_test.ts` with **name** in camelCase
  (e.g. `mainE2e_test.ts`). `pipeline/config/*.ts`: base
  name camelCase or PascalCase per §E.
