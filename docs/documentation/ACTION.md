...
Exceptions only via
sharepoint/context/scripts/checkLineLengthConfig.ts; do not add
...
- **Line length**: Each line must be ≤ 100 characters
  ...
  exemptions.
- **Enforcement**:
  sharepoint/context/scripts/checkLineLength.ts; run via
...
- **Line/file-length check**:
  sharepoint/context/scripts/checkLineLength.ts; config:
  sharepoint/context/scripts/checkLineLengthConfig.ts.
...
Files under `sharepoint/context/scripts/*.ts` use camelCase or
