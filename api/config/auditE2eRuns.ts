//
// E2E run artifacts under application/governance/ (e2e_runs.toml); not served.
//
...
const E2E_RUNS_PATH = `${LOG_DIR}e2e_runs.toml`;
const MAX_RUNS = 20;
...
export async function readE2eRuns(): Promise<E2ERunsFile> {
  const data = await readTomlFile<E2ERunsFile>(
    E2E_RUNS_PATH,
  );
  return data !== null
    ? normalizeRuns(data)
...
export async function appendE2eRun(
  entry: E2ERunEntry,
): Promise<void> {
