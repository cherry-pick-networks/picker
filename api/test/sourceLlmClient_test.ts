//
// sourceLlmClient: mock returns fixed output; no API key returns error.
//

import { assertEquals } from '@std/assert';
import { extractConcepts } from '#api/storage/catalog/sourceLlmClient.ts';
...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      if (result.ok) {
        assertEquals(result.output.concept_ids, [
          'bloom-1',
        ]);
...
Deno.test(
  'extractConcepts without mock and no API key returns ok: false',
  async () => {
