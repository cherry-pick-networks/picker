//
// Build keyword → source_id pairs from material_sources.toml + .env (RULESET.md §V).
// Used by material (source) for utterance matching; no hardcoded book names.
//

import { parse } from '@std/toml';

const MATERIAL_SOURCES_TOML = new URL(
  '../../../sharepoint/infra/seed/material/material_sources.toml',
  import.meta.url,
);
...
function loadTomlEntries(): SourceEntry[] {
  const raw = Deno.readTextFileSync(MATERIAL_SOURCES_TOML);
...
function buildPairsForMeta(
  ...
  const out: [string, string][] = title
    ? [[title, sourceId]]
    : [];
  for (const kw of kws) {
    if (typeof kw === 'string' && kw) {
      out.push([kw, sourceId]);
    }
  }
...
export function getLexisSourceKeywordPairs(): [
  string,
  string,
][] {
